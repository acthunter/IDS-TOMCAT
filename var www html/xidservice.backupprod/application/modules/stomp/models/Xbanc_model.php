...
class Xbanc_model extends CI_Model {
	...
	public function __construct()
	{
		...
		$this->load->library('banclib', $param); 
		//$this->load->database();
	}
	...
	function get_banc_login($id)
	{
		...
		$breply = $this->banclib->getLoginDetail($id);
		
		log_message('debug', print_r(array("xbanc_model.gbl.breply"=>$breply), TRUE));
		$filterStr = 'status,DefInteger4-npp,DefaultString1-name,BranchNo-branchcode,DefaultString2-loginStatus,DefInteger11-level';
		
		$retreply = $this->filterBanc($breply, $filterStr);
		log_message('debug', print_r(array("xbanc_model.gbl.retreply"=>$retreply), TRUE));
	...
	function get_tellparam($loginid, $effDate)
	{
		...
		$breply = $this->banclib->getLoginParam($loginid, $effDate);
		$this->slog->write_log('DEBUG', array("g.te.param.breply" =>print_r($breply, TRUE))); 
		...
		$this->slog->write_log('DEBUG', print_r($retreply, TRUE)); 
		return $retreply;
	}
