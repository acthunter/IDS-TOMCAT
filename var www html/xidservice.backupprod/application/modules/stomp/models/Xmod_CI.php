...
class Xmod_CI extends CI_Model {
	...
	public function createnew(){
		...
		/* $cek = $this->db->query("select accOffice2 as acc from xlogin xl join xposition xp on xl.positionid = xp.positionid where xl.loginid =".$myid)->row();
	...
	public function wfaction(){
		...
		if (preg_match('/save/',$fdt['reqtype'])){
			...
			$dataarr = array('loginid'=>$fdt['loginid'], 'mode'=>$fdt['mode'], 'positionid'=>$fdt['cposid'], 'pname'=>$fdt['cposname'], 'nama'=>$fdt['nama'], 'DOB'=>date('d-m-Y',strtotime($fdt['DOB'])), 
				'sdate'=> date('Y-m-d G:i',strtotime($sdate)), 
			);
			//var_dump($dataarr);
			$datastr = json_encode($dataarr);
			$accoffice = $_SESSION['pengguna']->accoffice;
			$xcp = array('type'=>'CI', 'accoffice'=>$accoffice,'status'=>'I',
					'data'=> $datastr);			
