...
class Xmod_UA extends CI_Model {
	...
	public function wfdetail($id){
		if ($id!=null){
			...
			if ($wf != null){
				...
				log_message('debug', print_r(array('masuksini '=>$rpos), TRUE));
				if (isset($rpos->data)){
				...
				log_message('debug', print_r(array('hasil iniii'=>$dataarr), TRUE));
				$wf->detail = $rpos;
			}
		} else {
			$wf = (object) array();
	...
	public function wfaction(){
		...
		if (preg_match('/save/',$fdt['reqtype'])){
			...
			$dataarr = array('npp'=>$fdt['npp'],'loginid'=>$fdt['loginid'], 'mode'=>$fdt['mode'], 'nama'=>trim(strtoupper($fdt['nama'])), 'nik'=>$fdt['nik'], 'address'=>$fdt['address'], 
				'positionid'=>$fdt['positionid'], 'pname'=>$fdt['pname'],'email'=>trim($fdt['email']), 'mobileNumber'=>$fdt['mobileNumber'],
		...
		if (preg_match('/(submit|approve)/',$fdt['reqtype']))
			...
			//log_message('debug', print_r(array("ua.wfaction.wfdetail"=>$wfdetail), TRUE)); 
			$dataarr = array('npp'=>$fdt['npp'],'loginid'=>$fdt['loginid'],'nama'=>trim(strtoupper($fdt['nama'])), 'mode'=>$fdt['mode'], 'nik'=>$fdt['nik'], 'address'=>$fdt['address'], 
				'positionid'=>$fdt['positionid'], 'pname'=>$fdt['pname'],'email'=>trim($fdt['email']), 'mobileNumber'=>$fdt['mobileNumber'],
				'DOB'=> date('d-m-Y',strtotime($fdt['DOB'])),
				);
			$datastr = json_encode($dataarr);
