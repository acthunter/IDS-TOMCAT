...
class Xmod_RP extends CI_Model {
	...
	public function jobitemlist(){
		...
		$this->_get_datatables_query($wfdetail, $tabel, $column_search, $column_order, $order);
		if($_POST['length'] != -1)
	...
	private function _get_datatables_query($wfdetail, $tabel, $column_search, $column_order, $order)
    {
	...
	public function createnew(){
		...
		if (!isset($row)){
			...
			$dataarr = array('submit'=>'', 'cancel'=>'', 'approve'=>'', 'reject'=>'');
			$datastr = json_encode($dataarr);
			$xrev = array ('wfid'=>$wfid, 'type'=>$type, 'accoffice'=>$accoffice, 
				'key1'=>$periode, 'status'=>$status_init, 'data'=> $datastr);
...
/* 	public function createnew(){
	...
	public function wfaction(){
		...
		if (preg_match('/(submit|approve|save)/',$fdt['reqtype']))
			...
			$query_ustr = $this->db->query("update xreviewpos set data = JSON_SET(data, '$.".$fdt['reqtype']."', '".$fdt['cttn']."') where wfid=${wfid}");
			$this->db->query($ustr);
			
		if (preg_match('/(tolak)/',$fdt['reqtype']))
			$direction = -1;
			$query3 = $this->db->query("update xreviewpos set data = JSON_SET(data, '$.reject', '".$fdt['cttn']."') where wfid=${wfid}");
		if (preg_match('/batal/',$fdt['reqtype'])){
			...
			$query3 = $this->db->query("update xreviewpos set status = 'C', key1='cancel', data = JSON_SET(data, '$.cancel', '".$fdt['cttn']."') where wfid=${wfid}");
