<script type="text/javascript">
	...
	function  <?php echo $modal_id;?>_trigger(fdata){
		...
		$('#dt_rp_list').on('click', 'tbody tr', function () {
				...
				//alert(rdata['id']);
				if(rdata['mode'] == "RP" || rdata['mode'] == "UH" || rdata['mode'] == "CI"|| rdata['mode'] == "UA"){
					var ftype = rdata['ftype'];
					var fparam = {'id' : rdata['id'], "mode" : rdata['mode'],
						'url':'jobbyid', 'reqtype' : 'read_list'};
					loadForm('modal_l2', fparam, true);
					...
					$('#loginid2').val(rdata['list_npp']);
					//$('#tposname').val(rdata['change']);
					$('[name="cposname2"]').text(rdata['def']);
					$('[name="nama"]').text(rdata['nama']);
					$('[name="tposname"]').val(rdata['change']);
					$('#sdate2').val(rdata['efektifdate']);
					switch (rdata['mode'])
					{
						case 'CU':
						$pname = 'Change Unit';
      ...
      <div class="modal-body">
        <form action="#"  id="<?php echo $modal_id;?>_form"  role="form">
			...
			<div class="form-group row"style="width:90%;margin-left:6%; ">
				...
				<p id="l-login">					
					<input disabled style="width: 20%" name="loginid2" id="loginid2" placeholder="Login ID" class="column-full form-control" type="text" onchange="query_id()" onclick="$('#loginid, #nama,#cposname2,#tposname, #tposid').val('').text('');$('#pos_def').css('display', 'none');">
					<label name="nama" class="labelisi" id="nama" ></label>
					<input disabled name="nama" id="nama" readonly type="hidden">
					<label for="nama"  class="labelisi" name="cposname2" id="cposname2" ></label>
					<input disabled name="cposname2" id="cposname2" readonly type="hidden">
				</p>
				<p >
				...
				<input name="sdate2" id="sdate2" placeholder="Tanggal Efektif" class="form-control" type="text" readonly>
