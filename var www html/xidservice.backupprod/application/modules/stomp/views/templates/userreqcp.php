<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		console.log(dattime1.getHours());
		console.log(offset);
		console.log(-(Intl.DateTimeFormat().resolvedOptions().timeZone/60));
...
/* 		$('[name="jam"]').attr("style", "display: none; width: 15%; float:left; ");
					...
					success: function(wdata){
						...
						if (wdata['stage'] != '1'){
							if (wdata['stage'] >= '3'){
								...
								var arr_ro = "tposname,sdate,edate,loginid,jam,jam2".split(",");
								$('#keterangan').hide();
								$('#ket').text(rdata['note']);
								console.log(arr_ro);
								for(cid of arr_ro){
								...
								if(ssda <= strDate){
									if ((ssda == strDate) && ((edt[1] != "00:00") || (edt[1] == "00:00")) && (sdt[1] != "00:00")){
						...
						if ((fdata['reqtype'] == "read_list") && (wdata['stage'] >= '3')){
							var arr_ro = "tposname,sdate,edate,loginid,jam,jam2,keterangan".split(",");
							//console.log(arr_ro);
							//$('#keterangan').hide();
							//$('#ket').text(rdata['note']);
							$('#exp').hide();							
	...
	function datetimepicker(){
		...
		var n = d.getHours();
		$('#sdate').datetimepicker({
            ignoreReadonly: true, widgetPositioning: { vertical: 'bottom' }, format: 'YYYY-MM-DD'
            });
         $('#edate').datetimepicker({
            ignoreReadonly: true, disabledDates: [yest], widgetPositioning: { vertical: 'bottom' }, format: 'YYYY-MM-DD'
        });
		$('#jam').datetimepicker({
		...
		$("#jam2").on("dp.change", function (e) {
		 ...
		 $("#sdate").on("dp.change", function (e) {
			...
			var noww = now.getFullYear()+"-"+m+"-"+dat;
			var dde = new Date();
				var strDate = dde.getFullYear() + "-" + (((dde.getMonth() + 1) < 10 ? '0' : '') + (dde.getMonth() + 1)) + "-" + (dde.getDate() < 10 ? '0' : '') + dde.getDate();
			
			if((sdate <= strDate) || (edate <= strDate)){
				if((sdate <= strDate) && (edate <= strDate)){
					$('#<?php echo $modal_id;?>_btn_submit').attr('disabled', false).css('pointer-events', 'auto');
					$('#exp').hide();
					$('#edate').data("DateTimePicker").minDate(noww).date(noww);
					$('#jam').data("DateTimePicker").minDate(new Date).date(new Date).maxDate(new Date(new Date().setHours(21,59,0,0)));
					
					$('#jam2').data("DateTimePicker").minDate(new Date(new Date().setHours((new Date().getHours()+1),new Date().getMinutes(),0,0))).date(new Date(new Date().setHours((new Date().getHours()+1),new Date().getMinutes(),0,0))).maxDate(new Date(new Date().setHours(22,0,0,0)));
						$('#jam,jam2').removeAttr('disabled');
						$('#jam2').attr('disabled', false);
					return $(this).data('DateTimePicker').defaultDate(noww).minDate(noww) ;
				}else if (sdate <= strDate){

					$('#<?php echo $modal_id;?>_btn_submit').attr('disabled', false).css('pointer-events', 'auto');
					$('#exp').hide();
					$('#edate').data("DateTimePicker").minDate(noww).date(edate);
					$('#jam').data("DateTimePicker").minDate(new Date).date(new Date).maxDate(new Date(new Date().setHours(21,59,0,0)));
			...
			if (sdate.match(noww)){
				if (sdate.match(edate)){
					...
					$('#jam2').data("DateTimePicker").minDate(new Date(y.setHours((today.getHours()+1),today.getMinutes(),0,0))).date(new Date(d.setHours((today.getHours()+1),today.getMinutes(),0,0))).maxDate(new Date(new Date().setHours(22,0,0,0)));
        ...
        $("#edate").on("dp.change", function (e) {
			...
			if (sdate.match(noww)){
				...
				if (sdate.match(edate)){
					var today = new Date();
					var timejam = $('#jam').val()+ ':00';
					var dattim = new Date(sdate  + ' ' +  timejam);
					var y = new Date();
					y.setHours(dattim.getHours()+1,dattim.getMinutes(),0,0);
					var x = new Date();
					...
					$('#jam').data("DateTimePicker").minDate(x).date(new Date(ed)).maxDate(new Date(new Date().setHours(21,59,0,0)));	
					...
					$('#jam2').attr('disabled', true);
					$('#jam2').data("DateTimePicker").minDate(y).date(new Date(new Date().setHours(y.getHours(),(y.getMinutes()),0,0)));
					$('#jam').data("DateTimePicker").minDate(v).date(v).maxDate(new Date(new Date().setHours(21,59,0,0)));
		...
		<form action="#"  id="<?php echo $modal_id;?>_form"  role="form">
			...
			<div class="form-group row" style="width:90%;margin-left:6%;">				
				...
				margin-left:-30px;">
					...
					<li>Sebelum melakukan perubahan posisi di hari yang sama pastikan user yang akan diubah telah login icons telebih dahulu</li>
				</ul>
					...
					<input name="mode" id="mode" class="form-control" type="hidden" value="CT" >
