<script type="text/javascript">
			...
			$.ajax({
					...
					for (val of wdata["eaction"]){
							$("#<?php echo $modal_id;?>_btn_" + val).show();
										...
										if (data['srctype'] == 'sr'){
						$("#ok2, #ok4, #ok5, #ok6, #ok7").css("display","block");
						$("#nosr").val(data['key1']);
						$("#req").val(data['key0']);
						query_req2();
					...
					if (data['srctype'] == 'rm'){
						$(" #ok4, #ok3, #ok5, #ok6, #ok7").css("display","block");
						$("#inc").val(data['key1']);
						$("#req").val(data['key0']);
						query_req2();
						/* var s = new Date(data['doc_date']);
					...
					if (wdata['stage'] != '1'){
						...
						var arr_ro = "doc_date,nosr,req,inc".split(",");
							for(cid of arr_ro){
 			...
 			$.ajax({
				...
				success: function(data)
				{
					...
					$.each(data.opts, function (index, item) {
						//alert(item['apps']);
						$("#list_apps2").append("<tr class='checklisttr'><td>"+ item['apps'] +"</td><td><input type='radio' id ='b' value='rst' name="+item['apps'] +" ></td></tr>");
						  });
						 $("#ok_tabel2,  ok").show();
					/* if (data.opts != '') {
					$('#addlevel').hide();						
	...
	function <?php echo $modal_id;?>_submit(btype){
		$('#doc_date,#nosr,#inc,#req').removeAttr('disabled');
	...
	function query_req()
	{	
		var url = "<?php echo site_url('itservice/xmain/query_req')?>" ;
		$.ajax({
			url : url,
			type: "POST",
			data: {"npp": $('#req').val()},
		...
		<button type="button" class="close" data-dismiss="modal">&times;</button>
			...
			<div class="form-group row" style="width:90%;margin-left:6%;">
				...
				<p id="ok5" style="display:none;">
					<label style="color: rgb(64, 92, 96);">Requestor</label>
					<input type='text' id='req' name='req' class="form-control" placeholder="Requestor">
				</p>
				<p id="ok6" style="display:none;">
					<label style="color: rgb(64, 92, 96);">Name</label>
				...
				<p >
					<table id="example"style="text-align:center; width:100%;" class="table table-bordered">
						<thead  style="background-color: #b3b3b3;">
							<tr >
								<th style="text-align:center">Noaaaa</th>
