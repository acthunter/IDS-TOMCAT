...
/* 						$('#nama').val(rdata['nama']);
			...
			$('#loginid').click(function() {
				var arr_ro = "keterangan,tposname,sdate,edate,unit".split(",");
				for(cid of arr_ro){
			...
			$('#sdate').datetimepicker({
				  ...
				  widgetPositioning: { vertical: 'bottom' },
				   format: 'YYYY-MM-DD'
					...
					return $(this).data('DateTimePicker').defaultDate(today).minDate(today);
						});
			//alert(now);
			$('#jam').datetimepicker({
				ignoreReadonly: true,
	...
	function query_id()
		{		
			...
			$.ajax({
				...
				success: function(data)
				{
					...
					$('[name="uname"]').val(data.uname);
						...
						alert("User berada di cabang lain. Silahkan cek menu informasi user");
			...
			/* }else{
				...
				new PNotify({
					title: 'Notifikasi',
					text: 'Maaf untuk user tersebut tidak dapat perubahan unit. Silahkan mengajukan surat / memo ke Divisi ISU untuk perubahan unit user tersebut',
					type: 'error',
					styling: 'bootstrap3'
				});
			} */
		...
		<button type="button" class="close" data-dismiss="modal">&times;</button>
			...
			<div class="form-group row"style="width:90%;margin-left:6%;">
				...
				<ul style="font-size:12px;color: red;margin-bottom: 25px; margin-top: -23px;
					...
					<li>Perubahan antar unit / cabang akan efektif H+1</li>
				</ul>
