...
class Xctest extends CI_Controller {
	...
	public function loginlist(){
		...
		$accoffice4 = '718';
		//$accoffice5 = '60';
		$res = $this->db->query("select vln.name, vi.* from v_idmauth vi inner join v_login_name vln on vln.loginid = vi.loginid where vi.accoffice = ? OR vi.accoffice = ? OR vi.accoffice = ? OR vi.accoffice = ?", array($accoffice, $accoffice2, $accoffice3, $accoffice4))->result();
		echo json_encode(array("recordsTotal" => 1,
            "recordsFiltered" => 1,"data"=>$res));
	}	

	public function login(){
		$this->cas->getmycas_login(array('idm_init', 'idm_appr'));
		$fdt = $this->input->post();
