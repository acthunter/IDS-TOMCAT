...
/* 		$query = $this->db->query('select vlm.name, xw.id,  xw.stage, xw.mode, xw.initiator, xw.currActor, xw.doneActor, xw.currScore, xw.targetScore, xr.cdate 
	...
	public function initstage($wfid, $stage, $poutput){
		...
		log_message('debug', print_r(array("init.stage.wfparam"=>$wparam), TRUE)); 
			...
			if (isset($wparam[$stage]['target'])){
				...
				if ($stage >= 3){
					$uarr = array("req_flag"=>'P', "req_stat"=>'I');
					...
					$ustr = $this->db->update_string("xrequest", $uarr, "reqid=" . $wfid);
					//$this->db->query($ustr);
				}
				$nrow->pname = null;
			} 
	...
	public function getposunit($pattern){
		/* $data = $this->db->query("select *  from xentitas where name like '%" . $pattern . "%' or accOffice like '%" . $pattern . "%'");
		$crow = $data->result();
		return $crow; */
		$data = $this->db->query("select *  from xentitas where accOffice =?",array($pattern));
		//$data = $this->db->query("select *  from xentitas where name like '%" . $pattern . "%' or accOffice like '%" . $pattern . "%' or rubrik like '%" . $pattern . "%'");
		$crow = $data->result();
		return $crow;
	}
	public function getunit($pattern){
		/* $data = $this->db->query("select *  from xentitas where name like '%" . $pattern . "%' or accOffice like '%" . $pattern . "%'");
		$crow = $data->result();
		return $crow; */
		//$data = $this->db->query("select *  from xentitas where accOffice =?",array($pattern));
	...
	public function getunitbina($pattern, $loginid){ //INPUT UNIT UNTUK MENAMPILKAN KONDISI DARI POSISI
		$accoffice = $_SESSION['pengguna']->accoffice;
		if ( is_numeric($pattern) ) {
				$pattern = ltrim($pattern, '0');;
		}
		$data = $this->db->query("select xpos.positionid, xpos.name as posisi, xen.name as unit, xen.accOffice  from xentitas xen join xposition xpos on xen.accOffice = xpos.accOffice where xpos.jobid = '999999' and (xen.name like '%" . $pattern . "%' or xen.accOffice = '" . $pattern . "')");
		
		$crow = $data->result();
		return $crow;
	}
	
	public function getposunit2($pattern){
	...
	public function getapp($pattern, $loginid){
		$accoffice = $_SESSION['pengguna']->accoffice;
		$data = $this->db->query("select * from apps where name like '%" . $pattern . "%'");
