...
class Xmod_CP extends CI_Model {
	...
	public function wfaction(){
		...
		if (preg_match('/save/',$fdt['reqtype'])){
			...
			$dataarr = array('loginid'=>$fdt['loginid'], 'mode'=>$fdt['mode'], 'tpos'=>$fdt['tposid'], 'pname'=>$fdt['tposname'], 'nama'=>$fdt['nama'], 'cposname'=>$fdt['cposname'],
		...
		if (preg_match('/submit/',$fdt['reqtype'])){
			...
			$sdat = $fdt['sdate'];
		$jam = $fdt['jam'];
		$sdate = date('Y-m-d H:i',strtotime("$sdat $jam") );	
