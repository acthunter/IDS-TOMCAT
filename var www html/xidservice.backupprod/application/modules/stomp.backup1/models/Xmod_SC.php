...
class Xmod_SC extends CI_Model {
...
/* 			$sdat = $fdt['sdate'];
			...
			$dataarr = array('loginid'=>$fdt['loginid'],  'sco'=>$fdt['scoid'], 'nama'=>$fdt['nama'], 'mode'=>$fdt['mode'],'cposname'=>$fdt['cposname'], );
			$datastr = json_encode($dataarr);
			$accoffice = $_SESSION['pengguna']->accoffice;
			$xcp = array('type'=>'SC', 'accoffice'=>$accoffice,'status'=>'I',
					'data'=> $datastr);			
...
/* 			$sdat = $fdt['sdate'];
		...
		$dataarr = array('loginid'=>$fdt['loginid'], 'sco'=>$fdt['scoid'], 'nama'=>$fdt['nama'], 'cposname'=>$fdt['cposname'],);
