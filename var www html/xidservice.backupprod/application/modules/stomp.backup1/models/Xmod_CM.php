...
class Xmod_CM extends CI_Model {
	...
	public function wfaction(){
		...
		if (preg_match('/save/',$fdt['reqtype'])){
			...
			$dataarr = array('loginid'=>$fdt['loginid'], 'hp'=>$fdt['hp'], 'nama'=>$fdt['nama'], 'mode'=>$fdt['mode'], 'cposname'=>$fdt['cposname']); 
			$datastr = json_encode($dataarr);
			$accoffice = $_SESSION['pengguna']->accoffice;
			$xcp = array('type'=>'CM', 'accoffice'=>$accoffice,'status'=>'I',
					'data'=> $datastr);			
		...
		if (preg_match('/approve/',$fdt['reqtype'])){
			...
			$edate = $edate;
			$unit = 'test_unit';
			$target = "Perubahan No HP";
			$name = $dt_init->row()->name;
			$reqid = $wf->id;
			//$test = $wf[0];
			log_message('debug', print_r(array('wfaction.submit.target'=>$target), TRUE));
			$this->load->library('sendmail');
			$this->sendmail->submit_apprv($target, $name, $reqid, $name_usr, $npp_usr, $cpos, $unit,  
							 $initiator, $name_init, $name_office, $code_office, $name_appr, $npp_appr);*/
		}
		if (preg_match('/submit/',$fdt['reqtype'])){
			...
			log_message('debug', print_r(array('wfaction.submit.target'=>$target), TRUE));
			$this->load->library('sendmail');
			$this->sendmail->submit_rqst($target, $name, $reqid, $name_usr, $npp_usr, $cpos, $unit,  
							$initiator, $name_init, $name_office, $code_office); */
		}
			...
			/*$sdat = $fdt['sdate'];
				...
				$edate =  "$edat $jam2";;
		//var_dump($edate);*/
		$dataarr = array('loginid'=>$fdt['loginid'], 'hp'=>$fdt['hp'], 'nama'=>$fdt['nama'], 'cposname'=>$fdt['cposname']);
		...
		if ($direction == 0){
			...
			log_message('debug', print_r(array("CM.wfaction.ret.wfid"=>$direction), TRUE));
			$ret = $this->wfmodel->action($myid,$wfid, $direction, $fdt['notes']);
			log_message('debug', print_r(array("CM.wfaction.ret.2"=>$ret), TRUE));
		}
		return $ret;
	}
} 